...
/**
 * Implementa los métodos genéricos para los Daos
 */
public class GenericDao<E, PK extends Serializable> implements IGenericDao<E, PK> {
    ...
    @Override
    public List findAllOrderBy(String p)
    {
        try
        {
            return getSession().createCriteria(this.entityClass).addOrder(Order.asc(p)).list();
        }
        catch(HibernateException e)
        {
            throw new HibernateException(e);
